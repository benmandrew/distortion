...
Image::Image(const ImgData &data, u_int w, u_int h) {
    this->data = data;
    this->w = w;
    this->h = h;
}
...
Image Image::posterise(bool ignore_alpha = true) const {
    Image out(w, h);
    for (int i = 0; i < data.size(); i++) {
        const vec4 &v_o = data[i];
        vec4 &v_t = out.data[i];
...
Image Image::streak(const std::vector<int> h_iter,
                    const std::vector<int> v_iter,
                    const std::function<std::optional<int>(int, int, int, int, int)> get_streak_idx,
                    const std::function<int(const vec4&)> measure,
                    const std::optional<Image> &measure_source) const {
    Image out(w, h);
    for (int j : v_iter) {
        for (int i : h_iter) {
            size_t idx = j * w + i;
            const vec4 &v_o = data[idx];
            vec4 v_m = data[idx];
...
Image Image::streak_down(const std::optional<Image> &measure_source) const {
...
Image Image::streak_up(const std::optional<Image> &measure_source) const {
    std::vector<int> h_iter(w), v_iter(h);
    std::iota(h_iter.begin(), h_iter.end(), 0);
...
Image Image::streak_left(const std::optional<Image> &measure_source) const {
    std::vector<int> h_iter(w), v_iter(h);
    std::iota(h_iter.begin(), h_iter.end(), 0);
    std::iota(v_iter.begin(), v_iter.end(), 0);
    auto get_streak_pos = [](int w, int h, int i, int j,
...
Image Image::streak_right(const std::optional<Image> &measure_source) const {
    std::vector<int> h_iter(w), v_iter(h);
